...
Options:
    ...
    --train_file FILE        Training data file name.
    --valid_file FILE        Validation data file name.
...
class GGNNSparse(GGNN):
    ...
    def prepare_specific_graph_model(self) -> None:
        ...
        self.placeholders['adjacency_lists'] = [tf.placeholder(tf.int32, [None, 2], name='adjacency_e%s' % e) for e in range(self.num_edge_types)]
        self.placeholders['num_incoming_edges_per_type'] = tf.placeholder(tf.float32, [None, self.num_edge_types], name='num_incoming_edges_per_type')
                    ...
                    with tf.variable_scope('timestep_%i' % step):
                        ...
                        if self.params['use_edge_msg_avg_aggregation']:
                            num_incoming_edges = tf.reduce_sum(self.placeholders['num_incoming_edges_per_type'],
                                                               keepdims=True, axis=-1)  # Shape [V, 1]
    ...
    def gated_regression(self, last_h, regression_gate, regression_transform):
        ...
        return graph_representations  # [g] tf.squeeze()
    ...
    def make_minibatch_iterator(self, data: Any, is_training: bool):
        ...
        while num_graphs < len(data):
            ...
            while num_graphs < len(data) and node_offset + len(data[num_graphs]['init']) < self.params['batch_size']:
                ...
                for target_val in cur_graph['labels']:
                    if target_val is None:  # This is one of the examples we didn't sample...
                        target_task_values.append([-1., -1])
                        target_task_mask.append([0., 0.])
                    else:
                        target_task_values.append(target_val)
                        target_task_mask.append([1.,1.])
            ...
            #print(batch_target_task_values)
            batch_feed_dict = {
                ...
                self.placeholders['target_values']: np.transpose(batch_target_task_values, axes=[1, 0, 2]),
                self.placeholders['target_mask']: np.transpose(batch_target_task_mask, axes=[1, 0, 2]),
            ...
            #print(batch_feed_dict[self.placeholders['target_values']])
